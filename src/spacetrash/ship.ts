...
export class SpaceTrashShip extends EntityComponent {
  map: (Tile)[][];
  ...
  async addToMap(t: Tile): Promise<any> {
  ...
  async make() {
    ...
    for (let z = 0; z < this.shipSize-1; z++) {
      await this.addToMap(new WallTile(z, 0));
      await this.addToMap(new WallTile(this.shipSize - 1, z));
      await this.addToMap(new WallTile(z+1, this.shipSize - 1));
      await this.addToMap(new WallTile(0, z));
