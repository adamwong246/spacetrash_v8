...
export class ArcadePhysics {
   ...
   */
  private boot() {
    if (!this.world) this.world = new World(this.scene, this.config)
    if (!this.add) this.add = new Factory(this.world)
   ...
   */
  overlap(object1: any, object2: any, overlapCallback: any, processCallback: any, callbackContext: any) {
  ...
  closest(source, targets = this.world.bodies) {
    ...
    let closest: Body | StaticBody | null = null
    const x = source.x
    const y = source.y
  ...
  furthest(source, targets = this.world.bodies) {
    let max = -1
    let farthest: Body | StaticBody | null = null
    const x = source.x
    const y = source.y
    const len = targets.size
