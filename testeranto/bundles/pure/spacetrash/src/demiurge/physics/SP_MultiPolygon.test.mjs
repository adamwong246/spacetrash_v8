...
// src/demiurge/physics/SP_MultiPolygon.ts
var SP_MultiPolygon = class {
  polygons;
  ...
  addMultiPolygon(multPolygon) {
    this.polygons.push(...multPolygon.polygons);
  }
  union() {
    const ps = this.polygonsPolygonClippingStyle();
    console.log(
      "Input polygons for union (before filtering):",
      JSON.stringify(ps)
    );
    const filteredPs = ps.filter(
      (polygon) => polygon.some((ring) => ring.length >= 3)
    );
  ...
  difference() {
    ...
    const negativeSpaceResult = difference(
      ...filteredPs
      // fullMapPolygon,
    ...
    for (const multiPolygonElement of clippingStyle) {
      ...
      for (const ringCoords of polygon) {
        ...
        for (const c of ringCoords) {
          if (!Array.isArray(c) || c.length !== 2 || Number.isNaN(c) || Number.isNaN(c)) {
            console.error(
          ...
          satVectors.push(new SP_2d_Vector(c[0], c[1]));
        }
...
// src/demiurge/physics/SP_MultiPolygon.test.ts
var imp = {
  suites: {
  ...
  whens: {
    addPolygon: (polygon) => (mp) => {
      mp.addPolygons([polygon]);
      return mp;
    },
    addMultiPolygon: (other) => (mp) => {
...
var SP_MultiPolygon_test_default = Pure_default(null, spec, imp, interf);
export {
  SP_MultiPolygon_test_default as default,
  imp,
  spec
};
